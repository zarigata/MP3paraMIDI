...
## Features
- 🎚️ Demucs HTDemucs-based source separation for up to 6 stems (drums, bass, vocals, other, guitar, piano)
...
- ⚙️ Flexible model selection: choose 4-stem (htdemucs) for speed/quality or 6-stem (htdemucs_6s, default) for maximum separation

### Audio Separation Models

MP3paraMIDI uses Demucs HTDemucs v4 with multiple model options:

| Model | Stems | Quality | Speed | Use Case |
|-------|-------|---------|-------|----------|
| **htdemucs_6s** (default) | 6 | Good | Slower | Maximum instrument separation (drums, bass, vocals, other, guitar, piano) |
| htdemucs | 4 | Excellent | Faster | High-quality separation of core instruments |
| htdemucs_ft | 4 | Excellent+ | Slowest | Fine-tuned for highest quality on core instruments |
| hdemucs_mmi | 4 | Good | Fast | Multi-task model variant |

> **Note:** The guitar and piano stems generated by ``htdemucs_6s`` can be less reliable than the core four stems. Select ``htdemucs`` or ``htdemucs_ft`` when prioritising quality.

## Tech Stack
- Python 3.10+
- Flask 3.x
- FastAPI 0.115+
- PyTorch, Demucs HTDemucs v4 (``htdemucs_6s`` default with six stems)
  ...
  ```json
  {
    ...
    "data": {
      ...
      "stems": [
        ...
        {
          "name": "drums",
          "filename": "mix_stem_drums.wav",
          "size": 11223344,
          "download_url": "/api/download/8d6f3f7c-46a5-4d02-9240-ef7d63fb1d06/stems/mix_stem_drums.wav",
          "download_url_encoded": "/api/download/eyJqb2JfaWQiOiI4ZDZmM2Y3Yy00NmE1LTRkMDItOTI0MC1lZjdkNjNmYjFkMDYiLCJjYXRlZ29yeSI6InN0ZW1zIiwiZmlsZW5hbWUiOiJtaXhfc3RlbV9kcnVtcy53YXYifQ"
        },
        {
          "name": "bass",
          "filename": "mix_stem_bass.wav",
          "size": 9988776,
          "download_url": "/api/download/8d6f3f7c-46a5-4d02-9240-ef7d63fb1d06/stems/mix_stem_bass.wav",
          "download_url_encoded": "/api/download/eyJqb2JfaWQiOiI4ZDZmM2Y3Yy00NmE1LTRkMDItOTI0MC1lZjdkNjNmYjFkMDYiLCJjYXRlZ29yeSI6InN0ZW1zIiwiZmlsZW5hbWUiOiJtaXhfc3RlbV9iYXNzLndhdiJ9"
        },
        {
          "name": "other",
          "filename": "mix_stem_other.wav",
          "size": 8877665,
          "download_url": "/api/download/8d6f3f7c-46a5-4d02-9240-ef7d63fb1d06/stems/mix_stem_other.wav",
          "download_url_encoded": "/api/download/eyJqb2JfaWQiOiI4ZDZmM2Y3Yy00NmE1LTRkMDItOTI0MC1lZjdkNjNmYjFkMDYiLCJjYXRlZ29yeSI6InN0ZW1zIiwiZmlsZW5hbWUiOiJtaXhfc3RlbV9vdGhlci53YXYifQ"
        },
        {
          "name": "guitar",
          "filename": "mix_stem_guitar.wav",
          "size": 7766554,
          "download_url": "/api/download/8d6f3f7c-46a5-4d02-9240-ef7d63fb1d06/stems/mix_stem_guitar.wav",
          "download_url_encoded": "/api/download/eyJqb2JfaWQiOiI4ZDZmM2Y3Yy00NmE1LTRkMDItOTI0MC1lZjdkNjNmYjFkMDYiLCJjYXRlZ29yeSI6InN0ZW1zIiwiZmlsZW5hbWUiOiJtaXhfc3RlbV9ndWl0YXIud2F2In0"
        },
        {
          "name": "piano",
          "filename": "mix_stem_piano.wav",
          "size": 6655443,
          "download_url": "/api/download/8d6f3f7c-46a5-4d02-9240-ef7d63fb1d06/stems/mix_stem_piano.wav",
          "download_url_encoded": "/api/download/eyJqb2JfaWQiOiI4ZDZmM2Y3Yy00NmE1LTRkMDItOTI0MC1lZjdkNjNmYjFkMDYiLCJjYXRlZ29yeSI6InN0ZW1zIiwiZmlsZW5hbWUiOiJtaXhfc3RlbV9waWFuby53YXYifQ"
        }
      ]
    }
  }
...
+  ```
+
+> The number of stems returned depends on the selected model (4 for ``htdemucs``/``htdemucs_ft``/``hdemucs_mmi``, 6 for ``htdemucs_6s``).
+
+### POST /api/convert-to-midi
- **Example:**
  ```bash
...
- **Response:**
  ```json
  {
    ...
    "data": {
      ...
      "stems_converted": ["vocals", "drums", "bass", "other", "guitar", "piano"]
    }
  }
  ```
- **Example:**
...
- **Response:**
  ```json
  {
    ...
    "available_models": ["htdemucs", "htdemucs_ft", "htdemucs_6s", "hdemucs_mmi"],
    "supported_stems": ["vocals", "drums", "bass", "other", "guitar", "piano"]
  }
  ```

## Web UI

MP3paraMIDI includes a modern glassmorphism-inspired interface for uploading audio, previewing stems, and generating MIDI files without touching the API. The number of stems displayed depends on the selected Demucs model (4 or 6 stems).
...
- **One-click MIDI conversion** for all separated stems that were generated
...
3. **Listen** to each available stem with the integrated player controls
...
- **Model selection** dropdown with all supported Demucs variants (6-stem default)
- **Dynamic stem listing** that adapts to any number of separated stems with inline audio playback
...
- **Guitar or piano stems sound poor:** The ``htdemucs_6s`` model's additional stems are less reliable than the core four. Switch to ``htdemucs``/``htdemucs_ft`` or manually edit the MIDI output after conversion.
